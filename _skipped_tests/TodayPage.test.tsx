...
describe.skip("TodayPage", () => {
  it("renders freshness bar + pick cards + upcoming fixtures when there are picks", () => {
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={[opportunity("m1", 0.13)]} fixtures={[fixture("m1"), fixture("m2")]} dataFresh />,
    );
  ...
  it("caps pick cards at 5 and offers overflow button when onShowAll provided", () => {
    ...
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={seven} fixtures={[]} dataFresh onShowAll={() => {}} />,
    );
  ...
  it("hides overflow button without onShowAll", () => {
    const seven = Array.from({ length: 7 }, (_, i) => opportunity(`m${i}`, 0.1));
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={seven} fixtures={[]} dataFresh />,
    );
  ...
  it("shows stale empty state when data is not fresh (ignores opportunities)", () => {
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={[opportunity("m1", 0.13)]} fixtures={[fixture("m1")]} dataFresh={false} />,
    );
  ...
  it("shows no-fixtures state when there are no fixtures", () => {
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={[]} fixtures={[]} dataFresh />,
    );
  ...
  it("shows no-value state with fixture count when fixtures exist but nothing qualifies", () => {
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={[]} fixtures={[fixture("m1"), fixture("m2"), fixture("m3")]} dataFresh />,
    );
  ...
  it("limits upcoming fixtures to 3 and links each to its fixture deep link", () => {
    ...
    const markup = renderToStaticMarkup(
      <LandingPage {...baseProps} opportunities={[]} fixtures={five} dataFresh />,
    );
