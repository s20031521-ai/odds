# MASTER HANDOFF — odds-tool v1.0.1

> **俾接手 AI / 開發者：呢份文件係唯一入口。讀完呢份 + 按 §1.4 順序掃過引用文件，你應該可以完全 pickup 成個系統，唔使再問任何人。**
> 版本：v1.0.1（git tag `v1.0.1`，package.json version `1.0.1`）
> 日期：2026-07-21
> 語言：本 repo 所有交接文件用廣東話書寫，程式碼註解用英文。
> v1.0.1 變更：整合 team logos feature（`HANDOFF-2026-07-21-team-logos.md`，git commit `d4f835e`）入主交接文件。v1.0.0 文件保留做歷史。

---

## 0. 一句講晒

**odds-tool 係一個足球賠率價值分析 PWA，已正式上線生產：`https://odds.ballballchu.com.hk`。** React 19 + TS 前端（中文 UI + 球隊 logo）、raw `node:http` API、PostgreSQL 18、Cloudflare Tunnel 對外（零主機 port）、兩個 collector 自動運行緊。單一 owner 登入，冇 signup。四個投注模型（主客和／大細／角球／亞洲讓球）凍結喺 3% edge 門檻，全部 0 settled matches，等緊數據累積到每個 30 場先可以考慮調校。
...
|---|---|---|
| 1 | 本文件 | 全局 |
| 2 | `docs/HANDOFF-2026-07-21-team-logos.md` | 最新 feature：球隊 logo + 每日自動 deploy（含 stale SW 事故教訓） |
| 3 | `docs/KIMI-HANDOFF-2026-07-19.md` | Phase 2 部署全記錄 + VM 操作須知 |
| 4 | `docs/HANDOFF-2026-07-19-simple-dashboard-mode.md` | Dashboard 雙模式嘅做法範例 |
| 5 | `docs/runbooks/production-deployment.md` | 部署／rollback／smoke／secret rotation／故障 playbook |
| 6 | `docs/runbooks/local-postgres-development.md` | 本地 DB 開發 + 測試矩陣 |
| 7 | `docs/runbooks/legacy-migration.md` | 舊 archive → Postgres 遷移同 parity |
| 8 | `docs/CODEX-HANDOFF-2026-07-18.md` | 模型信任政策、snapshot 分類嘅歷史背景 |
| 9 | `.superpowers/sdd*/` | 每個 phase 嘅逐 task 證據（progress.md + task-*-report.md） |
...
| 測試 | 29 Vitest files / 192 tests 全綠（master，team logos 加咗 9 個）；14 個 node:test files；`tsc --noEmit` + `vite build` 通過 |
...
- `/team-logos/*` 同 `/team-logos.json` 由 Caddy `file_server` 直接派（打進 `dist/`，唔經 API）。
...
| `public/` | PWA icons + 舊 `hkjc-odds.json`（inert artifact，runtime 唔讀）+ `team-logos/`（70 PNG）+ `team-logos.json`（manifest，runtime 讀） |
...
|---|---|---|
| 前端 Vitest（29 files / 192 tests） | `node node_modules/vitest/vitest.mjs run` | 否 |
...
- 呢個流程（tarball→scp→rsync→pg_dump→tag rollback→build caddy→smoke）喺 team logos feature 期間多驗證咗三次（2026-07-20/21）。
...
| 2026-07-21 | **本文件** | v1.0.1 master handoff（整合 team logos） |

Git：base `645f22a chore: initial import`（2026-07-19 repo 重建）→ dashboard mode commits（`3d8480c`…`4e4cf0d`）→ v1.0.0 tag → team logos commits → `d4f835e docs: handoff for team logos feature` → **v1.0.1 tag 喺本文件 commit 上**。
