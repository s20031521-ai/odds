# MASTER HANDOFF — odds-tool v1.0.2
...
**odds-tool 係一個足球賠率價值分析 PWA，已正式上線生產：`https://odds.ballballchu.com.hk`。** React 19 + TS 前端（中文 UI + 球隊 logo + **Chiikawa pastel 主題**）、raw `node:http` API、PostgreSQL 18、Cloudflare Tunnel 對外（零主機 port）、兩個 collector 自動運行緊。單一 owner 登入，冇 signup。四個投注模型（主客和／大細／角球／亞洲讓球）凍結喺 3% edge 門檻，全部 0 settled matches，等緊數據累積到每個 30 場先可以考慮調校。
...
|---|---|---|
| 1 | 本文件 | 全局 |
| 2 | `docs/HANDOFF-2026-07-21-chiikawa-ui-refresh.md` | 最新 feature：Chiikawa UI 改造 + Playwright 修復（**已部署**，含收尾 commits 記錄） |
| 3 | `docs/HANDOFF-2026-07-21-team-logos.md` | 球隊 logo + 每日自動 deploy（含 stale SW 事故教訓） |
| 4 | `docs/KIMI-HANDOFF-2026-07-19.md` | Phase 2 部署全記錄 + VM 操作須知 |
| 5 | `docs/HANDOFF-2026-07-19-simple-dashboard-mode.md` | Dashboard 雙模式嘅做法範例 |
| 6 | `docs/runbooks/production-deployment.md` | 部署／rollback／smoke／secret rotation／故障 playbook |
| 7 | `docs/runbooks/local-postgres-development.md` | 本地 DB 開發 + 測試矩陣 |
| 8 | `docs/runbooks/legacy-migration.md` | 舊 archive → Postgres 遷移同 parity |
| 9 | `docs/CODEX-HANDOFF-2026-07-18.md` | 模型信任政策、snapshot 分類嘅歷史背景 |
| 10 | `.superpowers/sdd*/` | 每個 phase 嘅逐 task 證據（progress.md + task-*-report.md） |
...
- 入口：`index.html`（zh-Hant，`theme-color` `#FFF8F0`）→ `src/main.tsx` → `src/App.tsx`（1,134 行 monolith：auth state、data loading、settings、四模型 wiring）。
...
- **Chiikawa UI（v1.0.2 新）**：`src/components/Kawaii.tsx` — `<Mascot pose="chiikawa-corner|chiikawa-empty|momonga-loading|momonga-alert|login-duo" />` + `<KawaiiDecor />`（CSS 花瓣/星星，`aria-hidden`）；`momonga-alert` 係 40×40 icon 級，重用 momonga-loading 圖。`src/styles/kawaii.css` 管四個 pose 嘅定位/尺寸（corner 係 fixed 右下角常駐，mobile 縮細）+ decor 動效，`main.tsx` import。素材 `public/chiikawa/` 4 張 PNG，檔名係 plan contract，`Kawaii.tsx` 直接引用 `/chiikawa/<檔名>`，改名要兩邊一齊改。
- 樣式：`styles/tokens.css`（CSS variables 單一來源；**v1.0.2 重寫做 pastel 色板**，見 §5.6）、`layout.css`（nav active pastel pill）、`dashboard.css`（淺色底對比度修正）、`kawaii.css`、`styles.css`（pastel sweep；`@font-face` 引入 self-hosted Baloo 2）。
...
|---|---|---|
| 前端 Vitest（29 files / 196 tests） | `node node_modules/vitest/vitest.mjs run` | 否 |
...
| Playwright UI（4 viewports，mocks，32 tests） | `npm run test:ui:only` | 否 |
...
⚠️ **Playwright dashboard spec 嘅模式依賴（v1.0.2 修復後係顯式嘅）**：產品預設 dashboard 係「極簡」模式，但 `tests/ui/dashboard.spec.ts` 斷言「專業」模式 DOM。`9e28452` 之後 `mockApi` helper 用 `page.addInitScript` 每次導航前寫 `localStorage[DASHBOARD_MODE_STORAGE_KEY] = "pro"`（key 由 `src/dashboardMode.ts` import，唔係 hardcode；寫入失敗靜默回落產品預設）。init script **全 spec 生效** — 之後加 simple 模式斷言或改產品預設時記住呢點。
