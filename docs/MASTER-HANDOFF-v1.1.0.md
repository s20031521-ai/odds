# MASTER HANDOFF — odds-tool v1.1.0

> **俾接手 AI / 開發者：呢份文件係唯一入口。讀完呢份 + 按 §1.4 順序掃過引用文件，你應該可以完全 pickup 成個系統，唔使再問任何人。**
> 版本：v1.1.0（git tag `v1.1.0`，package.json version `1.1.0`）
> 日期：2026-07-21
> 語言：本 repo 所有交接文件用廣東話書寫，程式碼註解用英文。
> v1.1.0 變更：整合 **Today-first UI Phase A**（`HANDOFF-2026-07-21-today-first-ui-phase-a.md`，spec `8c2fb71` → plan `5b2ab7b` → SDD 13 task `4684a69`…`e837c02`）入主交接文件 — 新首頁 `#/today`（新鮮度條 + 三行精選盤卡 `<details>` 原地展開 + 三種廣東話冇貨狀態 + 即將開賽）、舊 `#/dashboard` alias 唔死、nav 改名 今日/賽程/分析/紀錄、`displayStake` helper、`today.css`、Playwright helpers 抽取 + today.spec，**已部署上 production**（2026-07-21 傍晚，bundle `index-BYUf2jMY.js`、備份 `pre-deploy-20260721-093231.dump`、rollback tag 已做）。純前端改動，模型檔 + `BuyDashboard.tsx` 零 diff。Phase B（單場分析頁）/ Phase C（賽程+紀錄執整、SimpleDashboard 刪除）未做。v1.0.0 / v1.0.1 / v1.0.2 文件保留做歷史。

---

## 0. 一句講晒

**odds-tool 係一個足球賠率價值分析 PWA，已正式上線生產：`https://odds.ballballchu.com.hk`。** React 19 + TS 前端（中文 UI + 球隊 logo + **Chiikawa pastel 主題** + **Today-first 首頁 v1.1.0**）、raw `node:http` API、PostgreSQL 18、Cloudflare Tunnel 對外（零主機 port）、兩個 collector 自動運行緊。單一 owner 登入，冇 signup。四個投注模型（主客和／大細／角球／亞洲讓球）凍結喺 3% edge 門檻，全部 0 settled matches，等緊數據累積到每個 30 場先可以考慮調校。
...
|---|---|---|
| 1 | 本文件 | 全局 |
| 2 | `docs/HANDOFF-2026-07-21-today-first-ui-phase-a.md` | 最新 feature：Today-first 首頁 Phase A（**已部署**，含 SDD 過程 + deferred minors） |
| 3 | `docs/HANDOFF-2026-07-21-chiikawa-ui-refresh.md` | Chiikawa UI 改造 + Playwright 修復（已部署） |
| 4 | `docs/HANDOFF-2026-07-21-team-logos.md` | 球隊 logo + 每日自動 deploy（含 stale SW 事故教訓） |
| 5 | `docs/KIMI-HANDOFF-2026-07-19.md` | Phase 2 部署全記錄 + VM 操作須知 |
| 6 | `docs/HANDOFF-2026-07-19-simple-dashboard-mode.md` | Dashboard 雙模式嘅做法範例（simple 模式已被 TodayPage 取代） |
| 7 | `docs/runbooks/production-deployment.md` | 部署／rollback／smoke／secret rotation／故障 playbook |
| 8 | `docs/runbooks/local-postgres-development.md` | 本地 DB 開發 + 測試矩陣 |
| 9 | `docs/runbooks/legacy-migration.md` | 舊 archive → Postgres 遷移同 parity |
| 10 | `docs/CODEX-HANDOFF-2026-07-18.md` | 模型信任政策、snapshot 分類嘅歷史背景 |
| 11 | `.superpowers/sdd*/` | 每個 phase 嘅逐 task 證據（progress.md + task-*-report.md） |
...
| 備份 | 手動 `pg_dump`：`/opt/odds-tool/backups/odds-2026-07-19.dump`（已驗證 10/10 表）+ Chiikawa 部署前 `pre-deploy-20260721-044351.dump` + `pre-deploy-20260721-055126.dump` + **Today-first 部署前 `pre-deploy-20260721-093231.dump`** — **冇自動備份** |
| 測試 | 35 Vitest files / **226 tests** 全綠（master；Today-first +30）；**Playwright 56/56 全綠**（4 viewports；today.spec 6 tests × 4 + dashboard.spec 32）；14 個 node:test files；`tsc --noEmit` + `vite build` 通過 |
| Dashboard | 雙模式：**今日（新預設，`TodayPage`）**+ 專業（`BuyDashboard` 原封），`#/today` 右上一條 toggle，localStorage `dashboard-mode`（值仍係 `"simple" \| "pro"` 冇變） |
| UI 主題 | Chiikawa pastel 主題 + **Today-first 首頁（v1.1.0 新，已上 production）**：新鮮度條 + 三行精選盤卡（`<details>` 原地展開：edge／模型 vs 莊家／建議注碼／alternatives）+ 三種廣東話冇貨狀態（數據舊／冇盤值博／冇波睇）+ 即將開賽 3 場；self-hosted Baloo 2 webfont；零外部資源（離線可用）；production bundle `index-BYUf2jMY.js`（2026-07-21 傍晚部署） |
| 路由 | `#/today`（首頁，新）/ `#/fixtures` / `#/analysis` / `#/history`；**舊 `#/dashboard` 喺 `pageFromHash` alias 落 today**（舊書籤/PWA 唔死），`#/dashboard/<id>` 深鏈維持落 fixtures；nav label：今日／賽程／分析／紀錄 |
...
- 入口：`index.html`（zh-Hant，`theme-color` `#FFF8F0`）→ `src/main.tsx` → `src/App.tsx`（~1,150 行 monolith：auth state、data loading、settings、四模型 wiring）。
- Hash router `src/route.ts`：`today` / `fixtures` / `analysis` / `history`（v1.1.0：bare `#/dashboard` alias 落 `today`，`#/dashboard/<id>` 深鏈維持落 fixtures）。
- `src/apiClient.ts`：`/api/v1` fetch wrapper，session/CSRF。
- Pages：`DashboardPage.tsx`（今日/專業 toggle）→ **`TodayPage.tsx`（v1.1.0 新首頁）**｜`BuyDashboard.tsx`（專業，**唔准改**，見 §10）；`AllFixtures.tsx`、`LoginPage.tsx`。`SimpleDashboard.tsx` **退役中**：已冇任何地方 import，連 test 留喺度，Phase C 先刪（見 §11.2）。
- **Today-first 元件（v1.1.0 新）**：`src/components/PickCard.tsx`（三行卡 + 原生 `<details>` 原地展開；export `formatSelection/formatOdds/formatKickoff`）、`FreshnessBar.tsx`（新鮮／過期／冇同步三態）、`EmptyState.tsx`（stale／no-fixtures／no-value 三種冇貨原因）、`src/stakeDisplay.ts`（`displayStake`：顯示用 fractional Kelly，defaults 1000/0.25/0.02 跟 analyzer）、`src/styles/today.css`（全部新 class，`main.tsx` import）。
...
- 樣式：`styles/tokens.css`（CSS variables 單一來源；pastel 色板，見 §5.6）、`layout.css`（nav active pastel pill）、`dashboard.css`（淺色底對比度修正）、`kawaii.css`、`today.css`（v1.1.0 新）、`styles.css`（pastel sweep；`@font-face` 引入 self-hosted Baloo 2）。
...
|---|---|---|
| 前端 Vitest（35 files / 226 tests） | `node node_modules/vitest/vitest.mjs run` | 否 |
...
| Playwright UI（4 viewports，mocks，56 tests） | `npm run test:ui:only`（**打 build 後 preview；src 改咗要先 `vite build`**） | 否 |
...
## 11. 已知問題／技術債（v1.1.0 時點）

### 11.1 未做（按優先順序）

1. **Today-first Phase B（單場分析頁）** — `#/analysis?match=<matchId>` 單場詳情容器 + 四市場卡；舊「模型健康」頁內容**直接刪除**（owner 2026-07-21 決定）；PickCard「睇單場分析」link 由 `#/fixtures/<id>` 改指 analysis。Spec：`docs/superpowers/specs/2026-07-21-today-first-ui-redesign-design.md` §2.4。
2. **Today-first Phase C（賽程+紀錄執整）** — 賽程頁分組/filter/搜尋/狀態點 + 紀錄頁 readiness 進度條 + **刪除退役中嘅 `SimpleDashboard.tsx` 連 test**（順手清理 PickCard helper 重複）。Spec §2.3/§2.5。
3. **自動備份（Phase 3）** — Restic 加密去 S3-compatible + 每日排程 + restore 演練。Owner 2026-07-19 話「唔一定要」；依家只靠手動 pg_dump。
4. **VM provider 快照** — owner 去控制台撳。
5. **閂 SSH 密碼登入** — key-only 已驗證可用；owner 未批。
6. **Cloudflare Access** — 可選加多層驗證。
7. **Phase 3 其他**：private GitHub + CI + GHCR + image scanning。
...
7. **Playwright spec 隱性模式依賴**（v1.0.2）：dashboard spec 斷言「專業」模式 DOM 但產品預設係「極簡」，之前靠測試順序/localStorage 殘留僥倖過，UI 改造郁咗渲染路徑就成批 fail。修法係 `mockApi` 加 `page.addInitScript` 顯式寫 `"pro"`（見 §8.2）— 之後測試環境嘅模式假設要顯式，唔好靠順序。
