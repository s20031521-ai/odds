...
export function nextCandidateKickoffDelay(candidates: BuyCandidate[], now: number): number | null {
  let nextKickoff = Number.POSITIVE_INFINITY;
  for (const candidate of candidates) {
    const kickoff = Date.parse(candidate.commenceTime);
    if (Number.isFinite(kickoff) && kickoff > now) nextKickoff = Math.min(nextKickoff, kickoff);
  }
  return Number.isFinite(nextKickoff)
    ? Math.min(nextKickoff - now + 1, MAX_TIMEOUT_DELAY)
    : null;
}

export function candidateSelectionRuntime(
  candidates: BuyCandidate[],
  now: number,
): { now: number; nextDelay: number | null } {
  return { now, nextDelay: nextCandidateKickoffDelay(candidates, now) };
}
...
export function selectBuyOpportunities(
  ...
  const grouped = new Map<string, { candidate: BuyCandidate; picks: Map<string, BuyPick> }>();
  ...
  for (const candidate of candidates) {
    ...
    if (group) {
      const current = group.picks.get(key);
      if (!current || comparePicks(pick, current) < 0) group.picks.set(key, pick);
    } else {
      grouped.set(candidate.matchId, { candidate, picks: new Map([[key, pick]]) });
    }
  }

  return [...grouped.values()]
    .map(({ candidate, picks }) => {
      const [primary, ...alternatives] = [...picks.values()].sort(comparePicks);
      return {
        matchId: candidate.matchId,
        homeTeam: candidate.homeTeam,
        awayTeam: candidate.awayTeam,
        ...(candidate.homeTeamZh ? { homeTeamZh: candidate.homeTeamZh } : {}),
        ...(candidate.awayTeamZh ? { awayTeamZh: candidate.awayTeamZh } : {}),
        commenceTime: candidate.commenceTime,
        ...(candidate.league ? { league: candidate.league } : {}),
        ...(candidate.leagueZh ? { leagueZh: candidate.leagueZh } : {}),
        primary,
        alternatives,
      };
    })
...
function pickKey(pick: BuyPick): string {
  return `${pick.market}\u0000${pick.line === undefined ? "" : pick.line}`;
}
